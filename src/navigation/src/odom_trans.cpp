...
tf::Quaternion RQ2;
float trans_x = 0.1, trans_y = 0.0, trans_z = 0;
...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    odom_broadcaster.sendTransform(tf::StampedTransform(transform, msg->header.stamp, "camera_init", "livox_frame"));
    //计算里程计给movebase flex
    odom_last = odom;
    ...
    odom_move_base.child_frame_id = "livox_frame";
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber lio_odom_sub = nh.subscribe("Odometry", 1000, &odom_callback);
